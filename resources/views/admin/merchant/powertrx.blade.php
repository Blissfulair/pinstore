        ...
        <div class="card content-area content-area-mh ">
            <div class="card-innr">
                <div class="card-head has-aside">
                    <h4 class="card-title">All Electricity Bills Transactions</h4>
                    <div class="card-opt">
                        ...
                        <div class="page-nav w-100 w-lg-auto">
                            <ul class="nav">
                             @foreach($network as $k=>$data)
                                <li class="nav-item"><a class="nav-link" href="{{route('admin.powerind', $data->code)}}">{{$data->name}}</a></li>
                             @endforeach
                                    <li class="nav-item"><a class="nav-link" href="{{route('admin.powersales')}}">All</a></li>

                            </ul>
                        </div>
                        <div class="search flex-grow-1 pl-lg-4 w-100 w-sm-auto">
                            <form action="{{route('admin.cabletvplansearch')}}" method="POST" autocomplete="off">
                            {{ csrf_field() }}
                ...
                <table class="data-table admin-tnx ">
                    <thead>
                        <tr class="data-item data-head">
                            ...
                            <th class="data-col dt-token">Company</th>
                            <th class="data-col dt-token">Meter Number</th>
                            <th class="data-col dt-token">Meter Token</th>
                            <th class="data-col dt-token">Amount Paid</th>
                    ...
                    @foreach($trx as $k=>$data)
                        <tr class="data-item" id="tnx-item-2">
                            ...
                            <td class="data-col dt-token">
                                <span class="lead token-amount">{{App\Power::whereCode($data->plan)->first()->name}}</span>
                                 <span class="sub sub-symbol">{{App\Power::whereCode($data->plan)->first()->slogan}}</span>

                            </td>
                            <td class="data-col dt-token">

                                <span class="lead amount-pay">{{$data->phone}}</span>
                                <span class="sub sub-symbol">@if($data->plan == 01)Prepaid @else Postpaid @endif <em class="fas fa-info-circle" data-toggle="tooltip" data-placement="bottom" title="Meter Type"></em></span>
                                                            </td>

                                                             <td class="data-col dt-token">

                                <span class="lead amount-pay">{{$data->accountnumber}}</span>
                                <span class="sub sub-symbol">Meter Token Generateds  <em class="fas fa-info-circle" data-toggle="tooltip" data-placement="bottom" title="Meter Type"></em></span>
                                                            </td>
                            <td class="data-col dt-token">
