...
public function transactionHistory(){
    $data['histories'] = Jamb::whereUser_id(auth()->id())->latest()->paginate(5);
...
public function purchasedScratchcards(){
    $histories = Bill::where(function($query){
        $query->whereUser_id(auth()->id())->whereType(3);
    })->orWhere(function($query){
        $query->whereUser_id(auth()->id())->whereType(4);
    })->latest()->paginate(5);
...
public function paymentTransactions(){
    $data['histories'] = Bill::whereUser_id(auth()->id())->latest()->paginate(5);
...
public function editUploadResult($id){
  ...
  public  function paystack_payment(Request $request)
   ...
   public function verifyPaystackPayment(){
    ...
    {
        $user = Auth::user();
        $trans = Mlmpay::whereTrx($reference)->first();
    ...
    function matrixupgrade()
    {
        ...
        if (Auth::guard('cbt')->user()->plan_id > 1 && Auth::guard('cbt')->user()->confirmed > 0) {
   ...
   $plan = Plan::find($request->plan_id);
    ...
    public function dashboard()
    {
        ...
        $data['used'] = Bill::whereStatus(1)->whereUserId($user->id)->sum('amount');
        $data['total_funded'] = withdrawLog::whereStatus(1)->whereCbtId($user->id)->sum('amount');
        $data['pending_withdraw'] = withdrawLog::whereStatus(0)->whereCbtId($user->id)->sum('amount');

        $data['active_jobs'] = Jamb::whereStatus(1)->whereCbt_id($user->id)->count();;
        $data['pending'] = Jamb::whereStatus(1)->whereType(1)->whereCbt_id($user->id)->count();
        $data['completed'] = Jamb::whereStatus(1)->whereType(2)->whereCbt_id($user->id)->count();
        $data['user'] = $user;
        ...
        return view('cbt.dashboard', $data);
    }
    ...
    public function authCheck()
    {
        ...
        if (Auth::guard('cbt')->user()->status == '1' && Auth::guard('cbt')->user()->email_verify == '1' && Auth::guard('cbt')->user()->sms_verify == '1') {
            return redirect()->route('home');
        } else {
  ...
  $to_name = $user->username;
    ...
    {
        $user = Cbt::findOrFail(Auth::guard('cbt')->user()->id);
...
//
  ...
  public function logtoggle(Request $request)
    ...
    public function changeOfInstitution()
    {
        ...
        $data['jamb'] = $jamb;
        return view('cbt.change-of-course', $data);
    }
    public function cbtUploads()
    {
        ...
        $data['jamb'] = $jamb;
        return view('cbt.uploads', $data);
    }
    public function cbtUpload($id)
    {
        $jamb = Jamb::find($id);
        $user = Auth::guard('cbt')->user();
        $jamb->cbt_id = $user->id;
        $jamb->update();
        $data['page_title'] = "Upload of O-Level/A-Level Result";
        $data['jamb'] = $jamb;
        return view('cbt.upload-details', $data);
    }
    public function changeOfInstitutionDetails($id)
    {
        $jamb = Jamb::find($id);
        $user = Auth::guard('cbt')->user();
        $jamb->cbt_id = $user->id;
        ...
        $data['page_title'] = "Change of course/Institution";
        $data['jamb'] = $jamb;
        return view('cbt.change-details', $data);
    }

    // public function apply(Request $request){
    //     $applied = Application::whereUser_id(Auth::guard('cbt')->user()->id)->whereJob_id($request->job_id)->first();
    //     if($applied)
    ...
    //     $input['user_id'] = Auth::guard('cbt')->user()->id;
    //     $input['status']=1;
...
$charge = $gate->fixed_charge + ($request->amount * $gate->percent_charge / 100);
 ...
 public function updatetransfer(Request $request)
    ...
    public function depositLog()
    {
        $user = Auth::user();
        $data['gate'] = Gateway::find(107);
        $data['payment'] = Bill::whereUser_id(Auth::guard('cbt')->user()->id)->whereType(2)->whereStatus(0)->first();
        $data['page_title'] = "JAMB Upload of Results";
    ...
    public function withdrawLog()
    {
        ...
        return view('cbt.withdraw', $data);
    }
    public function saveWithdraw(Request $request){
        $user = Auth::guard('cbt')->user();
        if($request->amount>$user->balance)
        return back()->withAlert('Insufficient fund in the wallet');
        else{
            $user->balance = $user->balance-$request->amount;
            withdrawLog::create([
                'cbt_id'=>$user->id,
        ...
        return redirect()->route('cbt.withdraw-log');
    }
    public function withdrawHistory(){
        $user = Auth::guard('cbt')->user();
        $data['histories'] = withdrawLog::whereCbt_id($user->id)->latest()->paginate(15);
        $data['page_title'] = "Withdraw Log";
        return view('cbt.withdraw-log', $data);
    }
    ...
    public function updateChangeOfCourse(Request $request){
        ...
        if($jamb->update($data)){
            return redirect()->route('cbt.change')->withSuccess('Completed successfully');
        }
        return redirect()->route('cbt.change')->withAlert('Sorry, we encounted an error.');
    }
    ...
    public function updateUpload(Request $request)
    {
     $this->validate($request,
        ...
        elseif($request->status == -1)
       {
           ...
           return back()->withAlert('Comment is required');
       }

        if($jamb->update($docm)){
            return redirect()->route('cbt.uploads')->withSuccess('Completed successfully');
        }
        return redirect()->route('cbt.uploads')->withAlert('Sorry, we encounted an error.');
    }
    public function cbtHistories(){
        $id = Auth::guard('cbt')->user()->id;
        $histories = Jamb::whereStatus(1)->whereCbt_id($id)->latest()->paginate(15);
        $data['page_title'] = "History";
        $data['histories']=$histories;
        return view('cbt.history', $data);
    }
    public function cbtWithdraw(){
        $id = Auth::guard('cbt')->user()->id;
        $histories = Jamb::whereStatus(1)->whereCbt_id($id)->latest()->paginate(15);
        $data['page_title'] = "Withdraw";
        $data['histories']=$histories;
