...
class UserManageController extends Controller
{
  ...
  public function usertickets()
    ...
    {
        $data['category'] = Category::whereStatus(1)->get();
        $data['page_title'] = "Publish Job";
        return view('admin.post.job', $data);
    }
    ...
    public function storeJob(Request $request)
    {
        $request->validate([
            ...
            [
                ...
                'job_cat.required' => 'Please select a  Category',
        ...
        if($request->hasFile('image')){
            $image = $request->file('image');
            $filename = 'job'.time().'.jpg';
            $location = 'assets/images/post/' . $filename;
            Image::make($image)->resize(730,450)->save($location);
            $in['image'] = $location;
        }
        $in['status'] =  1;
        $in['created_by']=auth()->user()->id;
        $res = Job::create($in);
        if ($res) {
    ...
    public function sendemail(Request $request)
    {
        ...
        $to_email = $to;
        $heading = "New Message";
        $data = array("name"=> "Customer","heading"=>"New Message", "body" => $request->emailMessage);
